...
const {user, viewAsCompany, setViewAsCompany} = useUserSession()
const {is_superuser = false, permissions = []} = user || {}
...
<template>
  <div class="toolbar ml-auto">
    <div v-if="is_superuser">
      <VControl style="width: 100px">
      <QueryMultiSelect
        path="companies"
        query-key="name"
        mode="single"
        placeholder="View as company"
        :modelValue="viewAsCompany"
        @change="setViewAsCompany($event && $event.id)"
      />
      </VControl>
    </div>
